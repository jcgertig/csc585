...
$(function () {
    ...
    downloadTemplate: function (o) {
        var rows = $();
        $.each(o.files, function (index, file) {
            var row = $('<li class="span3" id="image_' + file.image_id + '">' +
                (file.error ? '<div class="name"></div>' +
                    '<div class="size"></div><div class="error" ></div>' :
                      '<div class="thumbnail">' +
                        '<a href="' + file.url +'" rel="lightbox-pictures" class="image_' + file.id + '" title="<%= pic.description %>">' +
                          '<img src="" alt="">') +
                        '</a>' +
                        '<div class="caption">' +
            ...
            if (file.error) {
                ...
                row.find('.btn-delete')
                    .attr('href', '/artifacts/' + $("#artifactID").val() + '/images/' + file.image_id);
                row.find('.btn-show')
                    .attr('href', '/artifacts/' + $("#artifactID").val() + '/images/' + file.image_id + '/edit');
            }
            rows = rows.add(row);
        });
        return rows;
